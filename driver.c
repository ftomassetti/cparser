                                      ...
                                      compile_mode_t mode)
{
	...
	if (unit->type == COMPILATION_UNIT_C
	    ...
	    size_t      len        = obstack_object_size(&c_cpp_cppflags_obst);
	    const char *extraflags = obstack_finish(&c_cpp_cppflags_obst);
	    obstack_1grow(&c_cpp_cppflags_obst, ' ');
	    obstack_grow(&c_cpp_cppflags_obst, extraflags, len);
	    driver_add_flag(&c_cpp_cppflags_obst, "-std=%s",
